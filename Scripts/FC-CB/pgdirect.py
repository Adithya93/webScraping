...
def info_from_mail(em):
    ...
    global failed
    output_str = ""
    requrl = pg_base + em + pg_end + pg_key
    try:
...
def info_from_mails(em):
    ...
    if waiting > 0:
        waitmessage = "The following requests are pending:\n"
        for mail in pending:
            waitmessage += mail + "\n"
    ...
    if waiting > 0:
        ans = input("Would you like to follow up on the " + str(waiting) + " pending requests now? y/n")
        if ans.lower() == "y":
