...
FIELDSET = set(["Email", "url", "type", "facebook_handle", "facebook_likes", "geo_streetnumber", 
        ...
        for line in thisComp[1:]:
            ...
            if value[0] == '{' and value[-1] == '}':
                ...
                for pair in info:
                    ...
                    hisFields.append(field + '_' + pair[0])
            elif value[0] == '[' and value[-1] == ']':
                info = fromList(value)
                writeSafe('<p id = "' + field + '">' + ', '.join(info) + '</p>', page)
                hisFields.append(field)
            else:
                writeSafe('<p id = "' + field + '">' + value + '</p>', page)
                hisFields.append(field)
        if len(hisFields) < len(FIELDSET):
            hisSet = set(hisFields)
            missing = FIELDSET.difference(hisSet)
            print('Missing ' + str(len(FIELDSET) - len(hisFields)) + ' fields for ' + email + ' :\n' + str(missing))
            for field in missing:
                writeSafe('<p id = "' + field + '">' + 'None' + '</p>', page)
        page.write('</div>')
    page.write('</html>')
    page.close()
