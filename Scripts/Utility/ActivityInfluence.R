...
activityScore <- function(x) {
  activities <- c('TW_statusesCount_NUM', 'TW_favoritesCount_NUM', 'DaysSinceLastPost') 
  ...
  activitiesDF <- select(x, TW_statusesCount_NUM, TW_favoritesCount_NUM, DaysSinceLastPost)
  numAct <- sapply(activitiesDF, char2Nums)  
  # Negate days since last post 
  numActDF <- as.data.frame(numAct)
  numActDF$DaysSinceLastPost <- - numActDF$DaysSinceLastPost 
...
influenceScore <- function(x) {
  infs <- c('LI_CONNECTIONS_NUM', 'TW_Retweets_NUM', 'INFERRED_SALARY_LEVEL_NUM',
            'LI_Followers_NUM', 'TW_followersCount_NUM', 'TW_friendsCount_NUM', 
  ...
  infsDF <- select(x, LI_CONNECTIONS_NUM, TW_Retweets_NUM, INFERRED_SALARY_LEVEL_NUM,
                   LI_Followers_NUM, TW_followersCount_NUM, TW_friendsCount_NUM, 
                   FB_Friends_NUM, TW_listedCount_NUM)
